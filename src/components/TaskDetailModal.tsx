...
import { X, Calendar, Tag, User, Users, AlertCircle, FileText, Layers } from 'lucide-react';
...
export function TaskDetailModal({ task, onClose }: TaskDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-card text-card-foreground rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="gradient-hero px-6 py-4 flex items-start gap-4">
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-base font-bold text-white leading-snug line-clamp-3">{task.plannedActions}</h2>
          </div>
          <button onClick={onClose} className="text-white/70 hover:text-white mt-1 shrink-0"><X size={20} /></button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          ...
          {task.summary && <Field icon={<FileText size={15} />} label="Summary" value={task.summary} />}
          <Field icon={<User size={15} />} label="Dashboard Owner" value={task.dashboardOwner} />
          <Field icon={<Users size={15} />} label="Task Supporters" value={task.taskSupporters} />
        ...
        <div className="border-t border-border px-6 py-4">
          <button onClick={onClose} className="w-full gradient-hero text-primary-foreground font-semibold py-2.5 rounded-xl text-sm hover:opacity-90 transition-opacity">← Back</button>
        </div>
      </div>
    </div>
  );
