...
import { STATUS_COLORS, ALL_STATUSES } from '../types/actionPlan';
      ...
      <div className="bg-card rounded-2xl p-5 shadow-card">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={statusData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
            ...
            <Tooltip contentStyle={{ borderRadius: 8, border: 'none', boxShadow: '0 4px 16px rgba(0,0,0,0.12)' }} />
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={ownerStatusData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
            ...
            {usedStatuses.map((status) => (
              <Bar key={status} dataKey={status} stackId="a" fill={STATUS_COLOR_MAP[status] || STATUS_COLOR_MAP['Other']} name={status} radius={usedStatuses.indexOf(status) === usedStatuses.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
